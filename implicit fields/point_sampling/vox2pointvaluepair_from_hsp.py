...
for kkk in range(len(class_name_list)):
    ...
    voxel_input = "D:\\Datasets\\Shapenet_hsp\\shapenet\\modelBlockedVoxels256\\" + class_name + "\\"  # Francesco's directory
    ...
    for idx in tqdm(range(name_num)):
        ...
        for i in range(3, dim_voxel - 3):
            ...
            for j in range(3, dim_voxel - 3):
                ...
                for k in range(3, dim_voxel - 3):
                    if (batch_size_counter >= batch_size): break
                    if (np.max(voxel_model_64[i - 3:i + 4, j - 3:j + 4, k - 3:k + 4]) != np.min(
                            voxel_model_64[i - 3:i + 4, j - 3:j + 4, k - 3:k + 4])):
            ...
            for i in range(0, dim_voxel, 2):
                for j in range(0, dim_voxel, 2):
                    for k in range(0, dim_voxel, 2):
                        filled_flag = False
                        for (i0, j0, k0) in [(i, j, k), (i + 1, j, k), (i, j + 1, k), (i + 1, j + 1, k), (i, j, k + 1),
                                             (i + 1, j, k + 1), (i, j + 1, k + 1), (i + 1, j + 1, k + 1)]:
        ...
        for i in range(3, dim_voxel - 3):
            ...
            for j in range(3, dim_voxel - 3):
                ...
                for k in range(3, dim_voxel - 3):
                    if (batch_size_counter >= batch_size): break
                    if (np.max(voxel_model_32[i - 3:i + 4, j - 3:j + 4, k - 3:k + 4]) != np.min(
                            voxel_model_32[i - 3:i + 4, j - 3:j + 4, k - 3:k + 4])):
                        sample_points[batch_size_counter, 0] = i
                        sample_points[batch_size_counter, 1] = j
                        sample_points[batch_size_counter, 2] = k
